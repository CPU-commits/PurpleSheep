...
const heightFooter = computed(() => footer.value?.clientHeight)

onMounted(() => {
	header.value?.style.setProperty(
		'--footer-height',
		`${heightFooter.value?.toString()}px`,
	)
})
</script>
...
<template>
	<article class="PortfolioCard">
		...
		<NuxtLink :to="work.link" target="_blank">
			<header ref="header" class="PortfolioCard__header">
		...
		<footer ref="footer" class="PortfolioCard__footer">
